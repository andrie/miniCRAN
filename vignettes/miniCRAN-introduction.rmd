...
You will get a warning whenever there are multiple versions of a package saved in the repository. Currently, these duplicate versions need to be manually removed by the user before rebuilding the repository's package index. This last step is important, otherwise you may end up with a repo in an inconsistent state.
...
Update the versions of the packages currently stored in the miniCRAN repository. A prompt is given to confirm the update for each package. This prompt can be suppressed using `ask=FALSE`, which will update all packages. Be careful using this option if you want to keep certain packages at an older version.
